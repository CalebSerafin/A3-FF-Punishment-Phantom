/*
Function:
    CSSPunPh_fnc_punishment_dataGet
...
Examples:
    ...
    private _UID = "123";
    [_UID,_keyPairs] call CSSPunPh_fnc_punishment_dataGet; // Return values from key "test" and "420" with default as 0, in UID "123"
    [_UID,[]] call CSSPunPh_fnc_punishment_dataGet;    // Return all [key,value]s in UID "123"
    ["",[]] call CSSPunPh_fnc_punishment_dataGet;      // Return all UID strings
...
if !(typeName _UID == "STRING") exitWith {
    [1, format ["INVALID PARAMS | _UID=""%1""", _UID], _filename] remoteExecCall ["CSSPunPh_fnc_log",2,false];
    false;
};

private _data_namespace = call CSSPunPh_fnc_punishment_dataNamespace;
if (_UID == "") exitWith {
