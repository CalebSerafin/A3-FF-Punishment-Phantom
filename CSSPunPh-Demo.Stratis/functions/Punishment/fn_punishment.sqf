/*
Function:
    CSSPunPh_fnc_punishment

Description:
    Punishes the player given for FF.
    Doesn't do the checking itself, refer to CSSPunPh_fnc_punishment_FF.
...
Examples:
    [_instigator,_timeAdded,_offenceAdded,_victim] remoteExec ["CSSPunPh_fnc_punishment",2,false]; // How it should be called from another CSSPunPh_fnc_punishment_FF.
    // Unit Tests:
    [cursorObject, 0, 0] remoteExec ["CSSPunPh_fnc_punishment",2];                                 // Ping with FF Warning
    [cursorObject,120, 1] remoteExec ["CSSPunPh_fnc_punishment",2];                                // Punish, 120 additional seconds
    [player,10, 1] remoteExec ["CSSPunPh_fnc_punishment",2];                                       // Test Self Punish, 10 additional seconds
    // Function that goes hand-in-hand
    [cursorObject,"forgive"] remoteExec [CSSPunPh_fnc_punishment_release,2]; // Forgive all sins
...
([_UID,_keyPairs] call CSSPunPh_fnc_punishment_dataGet) params ["_offenceTotal","_lastTime"];
...
[_UID,_keyPairs] call CSSPunPh_fnc_punishment_dataSet;
...
if (_victim isKindOf "Man") then {
    ...
    if (CSSPunPh_FFPunTellVictim && isPlayer _victim) then {
        ["FF Notification", [_name," hurt you!"] joinString ""] remoteExec ["CSSPunPh_fnc_customHint", _victim, false];
    };
...
[2, ["WARNING | ",_name," [",_UID,"] ",_victimStats,", ",_playerStats] joinString "", _filename] call CSSPunPh_fnc_log;

if (CSSPunPh_FFPunTellInstigator) then {
    ["FF Warning", ["Watch your fire!",_injuredComrade,_customMessage] joinString "<br/>"] remoteExec ["CSSPunPh_fnc_customHint", _instigator, false];
};

if (CSSPunPh_FFPunTellAdmin) then {
    private _admin = [] call CSSPunPh_fnc_getAdmin;
    if (!isNull _admin) then {
        ["FF Notification", [_name," has been found guilty of FF.<br/>Total Offences: ",str _offenceTotal, "<br/>Victim: ",name _victim] joinString ""] remoteExec ["CSSPunPh_fnc_customHint",_admin,false];
    };
};
